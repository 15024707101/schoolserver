...
// encodeQueryInnerSingle 拼接查询字符串
...
// isSingleValue 判断是否是一个值
func isSingleValue(data reflect.Value) bool {
	if data.IsValid() == false {
		return true
	}
	dataKind := array.GetTypeKind(data.Type())
	if dataKind == array.TypeKind.ARRAY || dataKind == array.TypeKind.MAP {
		return false
	} else if dataKind == array.TypeKind.INTERFACE || dataKind == array.TypeKind.PTR {
...
// encodeQueryInnerArray 查询字符串拼接成数组
...
// encodeQueryInnerMap 查询字符串拼成map
...
// encodeQueryInner 拼接成查询字符串
func encodeQueryInner(prefix string, data reflect.Value) (string, error) {
	if data.IsValid() == false {
		return "", nil
	}
	dataType := data.Type()
	dataKind := array.GetTypeKind(dataType)
	if dataKind == array.TypeKind.BOOL ||
		dataKind == array.TypeKind.INT ||
		dataKind == array.TypeKind.UINT ||
		dataKind == array.TypeKind.FLOAT ||
		dataKind == array.TypeKind.STRING {
		return encodeQueryInnerSingle(prefix, fmt.Sprintf("%v", data))
	} else if dataKind == array.TypeKind.ARRAY {
		return encodeQueryInnerArray(prefix, data)
	} else if dataKind == array.TypeKind.MAP {
		return encodeQueryInnerMap(prefix, data)
	} else if dataKind == array.TypeKind.PTR ||
		dataKind == array.TypeKind.INTERFACE {
...
// EncodeUrlQuery 拼接url 参数
func EncodeUrlQuery(data interface{}) ([]byte, error) {
	dataMap := array.ArrayToMap(data, "url")
